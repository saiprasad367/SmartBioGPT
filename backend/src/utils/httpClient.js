 ...
 *
 * Every provider (UniProt, RCSB, ChEMBL, STRING, ...) gets its own instance so
 * that a failure in one does not open the breaker for the others.
 *
 * Features: per-provider timeout, bounded exponential backoff with jitter on
 * transient failures, and a circuit breaker that short-circuits calls while the
 * upstream is down.
 */
function createHttpClient(name, { baseURL, timeout = env.HTTP_TIMEOUT_MS, retries = env.HTTP_RETRIES, headers } = {}) {
