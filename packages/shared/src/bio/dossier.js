...
const chembl = require('./chembl');
const stringdb = require('./stringdb');
const structureProvider = require('./structure');

const cache = new Cache({
    namespace: 'dossier',
    max: 2000,
    ttlMs: Number(process.env.BIO_CACHE_TTL_MS) || 6 * 60 * 60 * 1000,
});

/**
 * Aggregate a normalized "protein dossier" from several public databases.
 *
 * UniProt is the spine (required). Every enrichment call runs concurrently and
 * independently via allSettled, so STRING or ChEMBL being down only removes
 * that section - the researcher still gets a useful result.
 */
